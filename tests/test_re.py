...
class ReTests(unittest.TestCase):
    ...
    def test_basic_re_sub(self):
        ...
        """
        back references are not supprted '\g<name>'
        ...
        """
        vertical spaces are not supprted '\v'
    ...
    def test_bug_1140(self):
        ...
        # Also:
        # re.sub(x, y, six.u(x)) should return six.u(y), and
        # re.sub(x, y, str(x)) should return
        #     str(y) if isinstance(y, str) else six.u(y).
        for x in 'x', u'x':
            for y in 'y', u'y':
                z = re.sub(x, y, u'')
                self.assertEqual(z, u'')
                self.assertEqual(type(z), six.text_type)
                #
                z = re.sub(x, y, '')
                self.assertEqual(z, '')
                self.assertEqual(type(z), str)
                #
                z = re.sub(x, y, six.u(x))
    ...
    def test_re_subn(self):
        ...
        # Pyre2 doesn't yet support custom replacement counts
    ...
    def test_sre_character_literals(self):
        ...
        self.assertRaises(re_error, re.match, "\911", "")
    ...
    def test_bug_581080(self):
        iter_r = re.finditer(r"\s", "a b")
        if six.PY2:
            self.assertEqual(iter_r.next().span(), (1,2))
            self.assertRaises(StopIteration, iter_r.next)
        else:
            self.assertEqual(next(iter_r).span(), (1,2))
            self.assertRaises(StopIteration, iter_r.__next__)

        """
        Pyre2 scanner not implemented
    ...
    def test_bug_817234(self):
        iter_r = re.finditer(r".*", "asdf")
        if six.PY2:
            self.assertEqual(iter_r.next().span(), (0, 4))
            self.assertEqual(iter_r.next().span(), (4, 4))
            self.assertRaises(StopIteration, iter_r.next)
        else:
            self.assertEqual(next(iter_r).span(), (0, 4))
            self.assertEqual(next(iter_r).span(), (4, 4))
            self.assertRaises(StopIteration, iter_r.__next__)

    """
    pyre2 does not support arrays
...
def run_re_tests():
    ...
    for t in tests:
        ...
        if len(t) == 5:
            ...
            raise ValueError('Test tuples should have 3 or 5 fields')
            ...
            if outcome == SYNTAX_ERROR:
                ...
                if result is not None:
                    ...
                    for i in range(1, 100):
                        ...
                        vardict['g%d' % i] = gi
                    """
                    groupindex is not implemented in pyre2
                ...
                # still succeeds.
                try:
                    result = obj.search(six.u(s, "latin-1"))
                    if result is None:
                ...
                # still succeeds.
                obj=re.compile(six.u(pattern, "latin-1"))
                result = obj.search(s)
                if result is None:
